...
- **Endpoints come from the CLI's auth state.** The only hardcoded URL is the MCP endpoint `https://api.strata.space/mcp` in `.mcp.json` (and the README's other-clients snippet). This is the canonical, region-free production endpoint. When changing it, update every reference and bump the plugin version. Document links in skills use `https://strata.space/app/documents/<docId>`.
